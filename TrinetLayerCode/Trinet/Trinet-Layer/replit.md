...
### CVE Scanner (Enterprise-Grade)
- **Backend**: Python FastAPI with async scan execution
- **Database**: PostgreSQL for scan persistence and findings storage
- **Scanner Engine**: ProjectDiscovery Nuclei for CVE detection
- **Templates**: Auto-synced from nuclei-templates repository (CVE 2025)
- **Located at**: `/cve-scanner` route

#### Backend Architecture (cve_scanner/)
- `main.py` - FastAPI app with CORS, database init, template sync on startup
- `api/routes.py` - REST endpoints: POST /api/scan, GET /api/scan/{id}, GET /api/admin/diagnostics
- `models/database.py` - SQLAlchemy models (Scans, Findings) with metadata fields
- `services/template_sync.py` - Git-based CVE template synchronization
- `services/nuclei_runner.py` - Nuclei CLI wrapper with template verification and debug mode
- `services/scan_orchestrator.py` - Job management, rate limiting, metadata propagation
- `utils/validators.py` - URL validation and security controls

#### Scan Verification & Metadata
- Template loading verification: counts YAML templates before scan, aborts if 0
- Scan metadata tracking: templates_loaded, requests_sent, matches_found, execution_time
- Debug mode for nuclei execution (backend-only, captures stdout/stderr)
- Admin diagnostics endpoint: nuclei version, template stats, last sync time

#### Security Controls
- Input validation blocks localhost, private IPs, file:// schemes
- Rate limiting: 5 requests/minute per IP
- Max concurrent scans: 3
- Detection-only scanning (no exploitation)
- Job timeout enforcement
- Audit logging

#### Frontend Features
- Target URL input with legal disclaimer
- CVE year selection (2025 enabled, 2026 placeholder)
- Real-time scan status polling with metadata display
- Severity-sorted findings display
- Expandable finding cards with CVE details
- Clear messaging when 0 CVEs found (explains CVE scan limitations)
